...
async function init(){
  ...
  if(window.FURCOLOR.demo)$$('#projectForm input,#projectForm select,#projectForm button').forEach(x=>x.disabled=true);
}
...
$$('[data-pick]').forEach(button=>button.onclick=async()=>{if(window.FURCOLOR.demo)return toast('演示模式不访问文件系统');try{const result=await api(`/api/local/pick/${button.dataset.pick}`,{method:'POST',headers:{'X-FurColor-Local':window.FURCOLOR.token}});if(result.path)button.parentElement.querySelector('input').value=result.path}catch(error){toast(error.message)}});
