...
from quality_engine import (analyze_exposure,apply_exposure_tone,apply_white_balance,
    enhance_eyes,estimate_neutral_white,load_eye_annotations,save_waveform,write_metrics)
...
def privacy_process(rgb:np.ndarray,faces:list[dict],include_review:bool)->np.ndarray:
    ...
    for face in faces:
        if face.get("suppressed_by_memory",False) or face.get("suppressed_by_geometry",False):continue
...
def main()->int:
    ...
    for i,rp in enumerate(recipes,1):
        try:
            ...
            exposure=analyze_exposure(balanced);toned=apply_exposure_tone(balanced,exposure)
            ...
            metrics={"file":src.name,"anchor":recipe["anchor"],"white_balance":wb,"exposure":exposure,"eye_masks":len(points),"watermark":str(watermark) if watermark else None}
    ...
    summary={"version":3,"generated_at":time.strftime("%Y-%m-%d %H:%M:%S"),"count":len(recipes),"output":str(output),"max_side":max_side,"jpeg_quality":quality,
             "pipeline":["anchor_style","neutral_white_balance","waveform_exposure_tone","eye_local_mask","human_face_privacy","watermark"],"selection":str(selection_path),"eye_annotations":str(eye_path),"counts":counts,
