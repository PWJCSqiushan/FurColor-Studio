...
默认面向 NVIDIA GPU；没有兼容显卡时可加 `-CpuOnly`，但批量速度会明显下降。首次安装需要下载数 GB 依赖，不能按“五分钟”估算。模型、论文、许可证和固定哈希见 [第三方声明](THIRD_PARTY_NOTICES.md)。
...
1. 已启用 Fursee 时点击“分析主体”，生成兽装框、匿名分组和新增人脸候选；
2. 点击“开始分析”，融合主体证据、隐私候选和参考样片；
3. 点击“打开人脸复核”，把候选标成真人脸或兽装误检；
4. 点击“标注兽装眼睛”，为每张照片分别绘制轮廓；
5. 点击“开始渲染”，生成 V4 AI 草稿；
6. 用图片浏览器检查草稿；
7. 完成四项质检并生成交付包。
...
- [4.0 模型卡与安全边界](docs/MODEL_CARD.md)
- [隐私政策](PRIVACY.md)
...
FurColor 原创源代码采用 [Apache License 2.0](LICENSE)。用户照片、水印、活动素材、私有标注、训练数据、人脸记忆和未随仓库分发的模型权重不自动包含在该授权中；Fursee、Ultralytics YOLO、DINOv3 及其衍生权重继续受各自条款约束，详见 [第三方声明](THIRD_PARTY_NOTICES.md)。
