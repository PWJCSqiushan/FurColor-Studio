...
def test_selection_semantics_delivery_gate_and_path_isolation(monkeypatch):
    ...
    try:
        ...
        manifest=root/"manifest.json";manifest.write_text(json.dumps({"items":[]}),encoding="utf-8")
        ...
        db.init()
        pid=services.create_project({"name":"test","source_dir":str(source),"edited_dir":str(edited),"analysis_dir":str(analysis),"output_dir":str(output),"watermark_path":"","manifest_path":str(manifest),"selection_mode":"positive"})
        ...
        db.run("UPDATE projects SET selection_mode='negative' WHERE id=?",(pid,))
        assert services.selected_stems(pid)=={"keep_me","default_me"}
        result=services.deliver(pid,["privacy","mask","exposure","watermark"],"delivery_test")
        assert result["count"]==2
        assert {x["file"] for x in result["files"]}=={"keep_me.jpg","default_me.jpg"}
